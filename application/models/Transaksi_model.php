...
class Transaksi_model extends CI_Model
{
    ...
    {
        if ($id == null) {
            $query = "select * from transaksi where status_pengiriman < 3";
            return $this->db->query($query)->result_array();
        } else {
            $query = "select * from transaksi where status_pengiriman < 3 and id_pengguna = " . $id;
            return $this->db->query($query)->result_array();
        }
    }
